...
// 성향 1: 공격적 전투 (빠른 보스 직행, 전투 노드 가중치 ↑)
vector<vector<double>> reward_aggressive = {
    {-100, 1, -1, 5, -1},  // 대기(0) -> 전투(3)로 갈 때 높은 보상(5)
    {-1, -100, 2, -1, 5},
    {-1, -1, -100, 3, 6},
    {-1, -1, -1, -100, 10},// 전투(3) -> 보스(4) 높은 보상(10)
    {-1, -1, -1, -1, -100}
};

// 성향 2: 안정적 파밍 (우회로, 상점/치유소 경유 가중치 ↑)
vector<vector<double>> reward_stable = {
    {-100, 5, -1, 1, -1},  // 대기(0) -> 상점(1)로 갈 때 높은 보상(5)
    {-1, -100, 6, -1, 2},  // 상점(1) -> 치유소(2) 높은 보상(6)
    {-1, -1, -100, 5, 2},  // 치유소(2) -> 전투(3) 높은 보상(5)
    {-1, -1, -1, -100, 10},
    {-1, -1, -1, -1, -100}
};
